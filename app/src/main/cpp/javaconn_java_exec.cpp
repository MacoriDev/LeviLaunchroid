...
static constexpr const char* APP_ROOT = "/data/data/org.levimc.launcher/files/JavaConnect";
static constexpr const char* APP_TMP = "/data/data/org.levimc.launcher/files/JavaConnect/tmp";
static constexpr const char* APP_JNA_TMP = "/data/data/org.levimc.launcher/files/JavaConnect/tmp/jna";
static constexpr const char* APP_PREFS = "/data/data/org.levimc.launcher/files/JavaConnect/prefs";
static constexpr const char* ROOT_DIR = "/sdcard/games/JavaConnect";
...
static void disablePointerTagging() {
...
#endif
    errno = 0;
    int prctlRc = prctl(PR_SET_TAGGED_ADDR_CTRL, 0, 0, 0, 0);
    int prctlErr = errno;
    errno = 0;
    int malloptRc = mallopt(M_BIONIC_SET_HEAP_TAGGING_LEVEL, M_HEAP_TAGGING_LEVEL_NONE);
    int malloptErr = errno;
    std::cerr << "JavaConnect wrapper: pointer tagging disable prctl=" << prctlRc
              << " errno=" << prctlErr
              << " mallopt=" << malloptRc
              << " errno=" << malloptErr << std::endl;
#endif
}
...
static bool mkdirs(const std::string& path) {
    ...
    for (char c : path) {
        cur += c;
        if (c != '/' || cur.size() == 1) continue;
        if (!dirExists(cur) && mkdir(cur.c_str(), 0700) != 0 && !dirExists(cur)) return false;
    }
    if (!dirExists(cur) && mkdir(cur.c_str(), 0700) != 0 && !dirExists(cur)) return false;
    return true;
}
...
int main(int argc, char** argv) {
    disablePointerTagging();
    mkdirs(APP_ROOT);
    mkdirs(APP_TMP);
    mkdirs(APP_JNA_TMP);
    mkdirs(APP_PREFS);
    setenv("HOME", APP_ROOT, 1);
    setenv("TMPDIR", APP_TMP, 1);
    chdir(ROOT_DIR);
    ...
    std::cerr << "JavaConnect wrapper: loading libjvm=" << libjvm << std::endl;
    ...
    if (jar.empty() || !fileExists(jar)) {
        std::cerr << "JavaConnect wrapper: jar missing: " << jar << std::endl;
        return 127;
    }

    const char* overridePrefixes[] = {
        "-Duser.home=",
        "-Djava.io.tmpdir=",
    ...
    vmOptStrings.erase(std::remove_if(vmOptStrings.begin(), vmOptStrings.end(), [&](const std::string& opt) {
        for (const char* prefix : overridePrefixes) {
            if (startsWith(opt, prefix)) return true;
        }
        return false;
    }), vmOptStrings.end());
    ...
    addOpt(std::string("-Duser.home=") + APP_ROOT);
    addOpt(std::string("-Djava.io.tmpdir=") + APP_TMP);
    addOpt(std::string("-Dio.netty.native.workdir=") + APP_TMP);
    addOpt(std::string("-Djna.tmpdir=") + APP_JNA_TMP);
    addOpt("-Djna.nosys=true");
    addOpt(std::string("-Djava.util.prefs.userRoot=") + APP_PREFS);
    addOpt("-Dlog4j2.disable.jmx=true");
    ...
    JavaVM* vm = nullptr;
    JNIEnv* env = nullptr;
    std::cerr << "JavaConnect wrapper: calling JNI_CreateJavaVM jar=" << jar << std::endl;
    ...
    std::cerr << "JavaConnect wrapper: JNI_CreateJavaVM OK" << std::endl;

    std::cerr << "JavaConnect wrapper: loading jar main" << std::endl;
