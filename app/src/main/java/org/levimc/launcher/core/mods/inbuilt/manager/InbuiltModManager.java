...
public class InbuiltModManager {
    ...
    private static boolean isRemovedModId(String modId) {
        return ModIds.CAMERA_PERSPECTIVE.equals(modId)
            || ModIds.CHICK_PET.equals(modId)
            || ModIds.FPS_DISPLAY.equals(modId)
            || ModIds.SNAPLOOK.equals(modId);
    }
    ...
    private InbuiltModManager(Context context) {
        ...
        boolean removedDisabledMods = false;
        removedDisabledMods |= addedMods.remove(ModIds.CAMERA_PERSPECTIVE);
        removedDisabledMods |= addedMods.remove(ModIds.CHICK_PET);
        removedDisabledMods |= addedMods.remove(ModIds.FPS_DISPLAY);
        removedDisabledMods |= addedMods.remove(ModIds.SNAPLOOK);
        if (removedDisabledMods) {
            savePrefs();
        }
    }
    ...
    public boolean isModAdded(String modId) {
        return !isRemovedModId(modId) && addedMods.contains(modId);
    }
